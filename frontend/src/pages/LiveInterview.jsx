...
function VocalWaveform({ active }) {
  const bars = Array.from({ length: 28 }).map((_, i) => 0.25 + Math.abs(Math.sin(i * 0.7)) * 0.75);
  return (
    <div className="flex items-center justify-center gap-[3px] h-16 w-full px-8">
      {bars.map((h, i) => (
        <motion.span
          key={i}
...
export default function LiveInterview() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-surface via-white to-surface flex">
      {/* history sidebar */}
      <aside className="hidden md:flex flex-col w-56 shrink-0 border-r border-border bg-white/60 px-4 py-6">
      ...
      <div className="flex-1 flex flex-col">
        <header className="flex items-center justify-between px-6 md:px-10 py-5">
          ...
          <p className="text-xs text-muted">Question {questionNumber} of {totalQuestions}</p>
        </header>

        <main className="flex-1 px-6 md:px-10 pb-10 max-w-2xl mx-auto w-full flex flex-col">
          {/* moving vocals animation */}
          <div className="rounded-3xl border border-border bg-white shadow-card flex items-center justify-center py-6 mb-5">
          ...
          <div className="rounded-2xl border border-border bg-white overflow-hidden">
            <textarea
              ...
              className="w-full h-28 resize-none p-4 text-sm text-ink placeholder:text-muted focus:outline-none"
            />
          ...
          <div className="mt-6 flex items-center justify-between gap-3">
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              onClick={() => navigate("/")}
              className="text-sm font-medium rounded-full px-5 py-2.5 border border-border text-red-500 hover:bg-red-50 transition-colors"
            >
              End interview
            </motion.button>
            ...
            <motion.button
              ...
              className="flex items-center gap-2 text-sm font-semibold rounded-full px-7 py-3 bg-accent text-white shadow-glow hover:bg-accent-dark transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {isSubmitting && (
                <motion.span
              ...
              {isSubmitting ? "Scoring…" : "Score and improve the response"}
            </motion.button>
          </div>
        </main>
      </div>
